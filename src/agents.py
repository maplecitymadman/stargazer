...
class AgentSystem:
  ...
  !kubectl command - Execute kubectl command"""
            ...
            # kubectl commands - execute via subprocess with validation
            import subprocess
            try:
                from .command_validator import validate_kubectl_command
            except ImportError:
                from command_validator import validate_kubectl_command

            kubectl_cmd = command[1:].strip()

            # Validate command before execution
            is_valid, error_msg = validate_kubectl_command(kubectl_cmd)
            if not is_valid:
