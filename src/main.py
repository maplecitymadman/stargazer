...
@click.pass_context
def scan(ctx, continuous, interval):
    ...
    async def run_scan():
        ...
        while True:
            try:
                click.echo(f"\n🔍 Scanning cluster at {datetime.now().strftime('%H:%M:%S')}...")
                
                issues = await discovery.scan_all()
...
def health(ctx):
    ...
    async def run_health():
        discovery = ctx.obj['discovery']
        health = await discovery.get_resource_health()
